...
const updatebyidhome = async (req, res) => {
  try {
    ...
    const data = await connection.query(
      "update  web_home set slider_id=?, home_about=?, slug=?,title=?, meta_title=?, meta_desc=?,description=?,cover_url=?,breadcrumb_name=?, project_small_title=?, project_main_title=?, project_desc=?,cover_type=?,image=?,status=?,ip=? where home_id=?",
      [
        slider_id,
        home_about,
        slug,
